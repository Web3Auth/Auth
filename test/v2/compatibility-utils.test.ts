...
describe("compatibility-utils", () => {
  ...
  describe("fromLegacyRequest", () => {
    ...
    it("handles requests without params", () => {
      ...
      const request = fromLegacyRequest(legacyRequest as JRPCRequest);
  ...
  describe("makeContext", () => {
    ...
    it("includes non-JSON-RPC properties from request in context", () => {
      ...
      expect(Array.from(context.keys())).toStrictEqual(["extraProp", "anotherProp"]);
    });
  });
  ...
  describe("deserializeError", () => {
    ...
    describe("Error.isError", () => {
      ...
      let isError: ReturnType<typeof vi.fn>;
    ...
    it.each([42, true, false, null, undefined, Symbol("test")])("creates a new Error with stringified message for non-object values", (value) => {
    ...
    it("creates a plain Error when thrown value is an object without code property", () => {
      ...
      expect(result).toStrictEqual(new Error("test error message", { cause: thrownValue.cause }));
    });
