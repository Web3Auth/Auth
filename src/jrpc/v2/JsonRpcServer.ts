...
export class JsonRpcServer<Middleware extends MiddlewareConstraint = JsonRpcMiddlewareV2> {
  ...
  static _coerceRequest(rawRequest: unknown, isRequest: boolean): JsonRpcCall {
    ...
    if (hasProperty(rawRequest, "params")) {
      request.params = rawRequest.params as JRPCParams;
    }

    if (isRequest) {
      (request as JRPCRequest).id = getUniqueId();
    }
   ...
   */
  async handle(request: JRPCRequest, options?: HandleOptions<MergedContextOf<Middleware>>): Promise<JRPCResponse<unknown>>;
   ...
   * @param options.context - The context to pass to the middleware.
   */
  async handle(notification: JRPCNotification, options?: HandleOptions<MergedContextOf<Middleware>>): Promise<void>;
   ...
   * notification.
   */
  async handle(rawRequest: unknown, options?: HandleOptions<MergedContextOf<Middleware>>): Promise<JRPCResponse<unknown> | void>;
 ...
 */
function hasValidParams(rawRequest: Record<string, unknown>): rawRequest is { params?: JRPCParams } {
  if (hasProperty(rawRequest, "params")) {
