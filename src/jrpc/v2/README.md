...
## Differences from `JRPCEngine`

- **Middleware signature** — V1 uses `(req, res, next, end)`. V2 uses `({ request, context, next })` and returns the result directly instead of mutating `res`.
- **Immutable requests** — V2 deeply freezes request objects. Modifications go through `next({ ...request, method: 'new' })` instead of mutating `req` in place.
- **No shared response object** — V2 middleware never see or mutate a response object. Results flow via return values.
- **`MiddlewareContext`** — Inter-middleware data uses an append-only `context` Map instead of non-JSON-RPC properties on the request object.
- **Immutable results** — Returned result objects are deeply frozen. Middleware can replace a result by returning a new one from `next()`.
- **Typed middleware** — `JRPCMiddlewareV2` is generic over request type, result type, and context type, enabling compile-time safety for composed pipelines.
- **Factory construction** — Engines are created via `JRPCEngineV2.create({ middleware })` (private constructor) instead of `new JRPCEngine()` + `.push()`.
- **First-class notifications** — V2 distinguishes requests (`id` present) from notifications (`id` absent) at the type level, with different return-type constraints for each.
- **`JRPCServer`** — A dedicated server wrapper handles request coercion and error serialization, replacing the built-in validation in V1's `handle()`.
- **No event emitter** — `JRPCEngineV2` is a plain class, not a `SafeEventEmitter` subclass.

## Usage

> [!TIP]
> For the legacy `JRPCEngine`, see [its readme](../../../README.md).
>
> For how to migrate from the legacy `JRPCEngine` to `JRPCEngineV2`, see [Migrating from `JRPCEngine`](#migrating-from-jrpcengine).

```ts
import {
  JRPCEngineV2,
  type Json,
  type JRPCMiddlewareV2,
  type JRPCRequest,
  type MiddlewareContext,
} from '@web3auth/auth/jrpc';

type Middleware = JRPCMiddlewareV2<
  JRPCRequest,
  Json,
...
Alternatively, pass the engine to a `JRPCServer`, which coerces raw request
objects into well-formed requests, and handles error serialization:

```ts
const server = new JRPCServer({ engine, onError });
const request = { id: '1', jsonrpc: '2.0', method: 'hello' };
...
Use `asLegacyMiddleware()` to convert a `JRPCEngineV2` or one or more V2 middleware into a legacy middleware.
...
```ts
import { asLegacyMiddleware, JRPCEngineV2, JRPCEngine } from '@web3auth/auth/jrpc';

const legacyEngine = new JRPCEngine();

const v2Engine = JRPCEngineV2.create({
  middleware: [
    // ...
  ],
});
...
const middleware1: JRPCMiddlewareV2<JRPCRequest> = ({ request }) => {
  /* ... */
};
...
const middleware2: JRPCMiddlewareV2<JRPCRequest> = ({ context, next }) => {
  /* ... */
};

const legacyEngine2 = new JRPCEngine();
...
const engine = JRPCEngineV2.create({
  middleware: [
    ...
    async ({ request, context }) => {
      ...
      // Return a result without calling next() to end the request
      return 42;
    },
  ],
});
...
they are used. Middleware defined in this manner must use the `JRPCMiddlewareV2` type:

```ts
export const permissionMiddleware: JRPCMiddlewareV2<
  JRPCRequest,
  Json, // The result
...
Creating a useful `JRPCEngineV2` requires composing differently typed middleware together.
...
This works the same way for `JRPCServer.handle()`.
...
The context is itself exposed as the third generic parameter of the `JRPCMiddlewareV2` type.
...
- Handle either `JRPCRequest`, `JRPCNotification`, or both (i.e. `JsonRpcCall`)
  - It is okay to mix `JsonRpcCall` middleware with either `JRPCRequest` or `JRPCNotification`
...
```ts
const middleware1: JRPCMiddlewareV2<
  JRPCRequest,
  Json,
  MiddlewareContext<{ foo: string }>
> = /* ... */;

const middleware2: JRPCMiddlewareV2<
  JRPCRequest,
  Json,
  MiddlewareContext<{ bar: string }>
> = /* ... */;

const middleware3: JRPCMiddlewareV2<
  JRPCRequest,
...
> [!WARNING]
> Providing `JRPCRequest`- and `JRPCNotification`-only middleware to the same engine is
...
```ts
const middleware1: JRPCMiddlewareV2<JRPCNotification> = /* ... */;

const middleware2: JRPCMiddlewareV2<JRPCRequest> = /* ... */;
...
```ts
const subEngine = JRPCEngineV2.create({
  middleware: [
    ({ request }) => {
...
const mainEngine = JRPCEngineV2.create({
  middleware: [
...
```ts
const loggingEngine = JRPCEngineV2.create({
  middleware: [
    ({ request, next }) => {
...
The `JRPCServer` wraps a `JRPCEngineV2` to provide JSON-RPC 2.0 compliance and error handling. It coerces raw request objects into well-formed requests and handles error serialization.

```ts
import { JRPCEngineV2, JRPCServer } from '@web3auth/auth/jrpc';
...
> [!WARNING]
> It is possible to construct a `JRPCServer` that only accepts either requests or notifications,
> but not both. If you do so, it is your responsibility to ensure that the server is only used with the
> appropriate request objects. `JRPCServer.handle()` will not type error at compile time if you attempt to pass
> it an unsupported request object.

## Migrating from `JRPCEngine`

Migrating from the legacy `JRPCEngine` to `JRPCEngineV2` is generally straightforward.
There are a couple of pitfalls to watch out for:

### `MiddlewareContext` vs. non-JSON-RPC string properties

The legacy `JRPCEngine` allowed non-JSON-RPC string properties to be attached to the request object.
`JRPCEngineV2` does not allow this, and instead you must use the `context` object to pass data between middleware.
...
const createFooMiddleware =
  (processFoo: (req: JRPCRequest) => string) => (req, res, next, end) => {
    if (req.method === 'foo') {
...
In the legacy `JRPCEngine`, request and response objects are mutable and shared between all middleware.
In `JRPCEngineV2`, response objects are not visible to middleware, and request objects are deeply frozen.
...
const createBarMiddleware =
  (processBar: (req: JRPCRequest) => string) => (req, _res, next, _end) => {
    if (req.method === 'bar') {
...
See [Request modification](#request-modification) for how to modify the request object in `JRPCEngineV2`.

## Contributing

This package is part of the [Web3Auth Auth SDK](https://github.com/web3auth/Auth). Instructions for contributing can be found in the [repository README](https://github.com/web3auth/Auth#readme).
