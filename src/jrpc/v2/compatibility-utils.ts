 ...
 */
export const deepClone = <Type>(value: Type): DeepCloned<Type> => klona(value) as DeepCloned<Type>;
 ...
 */
export function fromLegacyRequest<Request extends JRPCRequest>(req: Request): Request {
  const request = {
 ...
 */
export function makeContext<Request extends Record<string | symbol, unknown>>(req: Request): MiddlewareContext {
 ...
 */
export function propagateToContext(req: Record<string, unknown>, context: MiddlewareContext<Record<string, unknown>>): void {
  Object.keys(req)
    .filter((key) => typeof key === "string" && !requestProps.includes(key) && !context.has(key))
    .forEach((key) => {
 ...
 */
export function propagateToRequest(req: Record<string, unknown>, context: MiddlewareContext): void {
  Array.from(context.keys())
    .filter(((key) => typeof key === "string" && !requestProps.includes(key)) as (value: unknown) => value is string)
    .forEach((key) => {
 ...
 */
function deserializeData(data: unknown, cause: unknown): OptionalDataWithOptionalCause {
...
export function deserializeError(thrown: unknown): Error | JsonRpcError<Json> {
  ...
  const code = typeof thrown.code === "number" && Number.isInteger(thrown.code) ? thrown.code : undefined;
